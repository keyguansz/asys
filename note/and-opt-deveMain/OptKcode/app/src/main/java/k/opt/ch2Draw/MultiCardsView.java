...
public class MultiCardsView extends View{
    ...
    public void onDraw(Canvas canvas) {
        ...
        KLogUtil.D("draw", String.format("clip bounds %d %d %d %d", clip.left, clip.top, clip.right, clip.bottom));
    ...
    protected void drawCardsWithotOverDraw(Canvas canvas, int index) {
        ...
        if (card != null && !canvas.quickReject(card.area, Canvas.EdgeType.BW)) {
            ...
            if (canvas.clipRect(card.area)) {
                KLogUtil.D("draw", "overdraw opt: draw cards index: " + index);
                Rect clip = canvas.getClipBounds();
                KLogUtil.D("draw", String.format("current clip bounds %d %d %d %d", clip.left, clip.top, clip.right, clip.bottom));
                card.draw(canvas);
            }
            canvas.restoreToCount(saveCount);
        }else{
    ...
    protected void drawCardsNormal(Canvas canvas, int index) {
        ...
        SingleCard card = cardsList.get(index);
        if (card != null) {
            drawCardsNormal(canvas, index - 1);//用递归有啥好？浪费内存
            KLogUtil.D("draw", "draw cards index: " + index);
            card.draw(canvas);
        }
    }
}
