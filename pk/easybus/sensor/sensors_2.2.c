...
void sGetProcessName(int pid, char *buffer)
{
    int fd;
            ...
            sensors_data_t*  values)
{
    ...
    printf("No sensor to return!!! pendingSensors=%08x", data->pendingSensors);
...
//    D("%s: data=%p", __FUNCTION__, dev);
    ...
    while (1) {
        /* read the next event */
        char     buff[256] = {0},namebuf[256] = {0};
        int      len = RecvSensorsData(data->events_fd, buff, sizeof buff-1);
        float    params[3];
        int64_t  event_time;
    ...
    data_type    =  getintL( ( char*)buff,    4  );
    sensors_type =  getintL( ( char*)(buff+4),4  );
    accuracy     =  getintL( ( char*)(buff+8),4  );
    params[0]    =  floatmySwitch( ( char*)(buff+12)  );
    params[1]    =  floatmySwitch( ( char*)(buff+16) );
    params[2]    =  floatmySwitch( ( char*)(buff+20) );

    D("read: data_type==%d sensors_type==%d accuracy==%d,x=%f,y=%f,z=%f",data_type,sensors_type,accuracy,params[0],params[1],params[2]);
        /* "acceleration:<x>:<y>:<z>" corresponds to an acceleration event */
        if ( (1 == sensors_type )&&(0 == data_type) ) {
...
//            continue;
        ...
        else if ( (3 == sensors_type )&&(0 == data_type) ) {
...
data__close(struct hw_device_t *dev) 
{
    ...
    if (data) {
        if (data->events_fd > 0) {
       //        printf("(device close) about to close fd=%d", data->events_fd);
                close(data->events_fd);
       }
       free(data);
    }
