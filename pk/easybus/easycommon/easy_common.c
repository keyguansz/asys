...
unsigned long easy_crc32(void *pvStartAddress, unsigned long dwSizeInBytes)
{

   unsigned long ulCRC;
   unsigned char *pbData;
   
   EB_LOGD("[%s.%d] size=%d\n", __FUNCTION__, __LINE__, dwSizeInBytes);
...
void easy_print_mem(char *msg, char *buf, int size)
{
    ...
    for (index = 0; index < size/10; index++) {
        EB_LOGD("%02x %02x %02x %02x %02x %02x %02x %02x %02x %02x \n", buf[index*10 + 0], 
    ...
    for (index = 0; index < size%10; index++) {
        EB_LOGD("%02x ", buf[(size/10)*10 + index]);
    }
    EB_LOGD("\n");
}

void easy_print(const char *fmt, ...)
{
    char logSwitch[EASY_BUS_BUFF_MAX_LEN] = {0};
    va_list ap;
    
    va_start(ap, fmt);
    vsnprintf(logSwitch, EASY_BUS_BUFF_MAX_LEN-1, fmt, ap);
    va_end(ap);
